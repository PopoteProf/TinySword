...
public class EditorConditionTriggerCustomPropertyDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.FindPropertyRelative("Type").enumValueIndex == 1)
        {
            ...
            Rect ComporatorRect = new Rect(position.x + 90 + (position.width - 90) / 3, position.y,
                (position.width - 90) / 3, 20);
            Rect RessourveValueRect = new Rect(position.x + 90 + ((position.width - 90) / 3) * 2, position.y,
                (position.width - 90) / 3, 20);
            ...
            EditorGUI.PropertyField(RessourveValueRect, property.FindPropertyRelative("RessourceValue"),
                GUIContent.none);
        }

        if (property.FindPropertyRelative("Type").enumValueIndex == 2)
        {
            Rect itemComparatorRect = new Rect(position.x + 95, position.y, 70, 20);
            Rect itemRect = new Rect(position.x + 170, position.y, position.width - 170, 20);
            EditorGUI.PropertyField(itemComparatorRect, property.FindPropertyRelative("ObjectComparator"),
                GUIContent.none);
